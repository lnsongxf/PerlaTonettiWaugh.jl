...
\paragraph{General Algorithm (Fixed Point)}

The iteration begins with an (irregular) grid of time-stops $\tau \in \R^S$ where $\tau_1 = 0$ and $\tau_S = T$, for a large $T$.

Guess an initial $\hat{E}$ such that $\Omega(0) = \Omega_0$ and $\Omega(T) = \Omega_T$ 

\begin{enumerate}
	\item Use the current $\hat{E}$ guess to solve for the $\Omega(t)$ function from \cref{sec:entry-ODE}
	\item With the $\Omega(t)$ function, solve the DAE with the ODE, the value matching, the free-entry residual, and the export threshold (i.e. of length $P + 3$).
	\begin{itemize}
		...
		\item Make sure the time-stops used for the $\hat{E}$ calculation are passed to the DAE solver
		\item Check to ensure that $E(t) > 0$ from the passed in function, and potentially pin it to $0$ if it does to ensure the DAE iteration works
	\end{itemize}
	\item Extract the $E(t)$ exactly at the time-stops used in the $\hat{E}$ guess from the DAE solution
	\item Convert the $E(t)$ to an implied $\hat{E}(t)$ by subtracting $\delta$ and then rescaling so that $\hat{E}(0) = -1$ and $\hat{E}(T) = 0$ by dividing everything by $-\hat{E}(0)$
	\item Stop the iteration when $\hat{E}$ converges.  Consider alternative fixed point iterations such as Anderson iteration.
\end{enumerate}
